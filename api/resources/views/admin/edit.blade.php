...
@section('content')
    ...
    <div class="container-fluid">
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <form action="{{ route('admin.update', $admin) }}" method="POST" class="col-6 mx-auto">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label for="name">Nama</label>
                        <input type="name" class="form-control @error('name') is-invalid @enderror" name="name" id="name"
                            value="{{ $admin->name }}" autofocus required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror"
                            value="{{ $admin->email }}" name="email" id="email" required>
                        @error('email')
                            <div class="alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control @error('role') is-invalid @enderror" name="role" id="role" required>
                            ...
                            <option value="2" {{ $admin->role == 2 ? 'selected' : '' }}>Admin Dispora</option>
                            <option value="3" {{ $admin->role == 3 ? 'selected' : '' }}>Admin Dishub</option>
                        </select>
                        @error('role')
                            <div class="alert-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 @error('permissions') is-invalid @enderror" name="permissions[]"
                            ...
                            @foreach ($permissions as $id => $permissions)
                                <option value="{{ $id }}"
                                    {{ in_array($id, old('permissions', [])) || $admin->permissions->contains($id) ? 'selected' : '' }}>
                                    {{ $permissions }}
                                </option>
                            @endforeach
                        </select>
